/**
 * Renders the final attack path verifier report for the Attack Path Verifier Agent.
 */
function AttackPathVerifierAgentReport({ entry, index, isExpanded, toggleEntry }) {
  if (entry.type !== 'attack_path_verifier_report') return null

  const r = entry.attack_path_verifier_report || {}
  const verdictClass = {
    'Attack path validated': 'danger',
    'Attack path partially validated': 'warning',
    'Attack path not feasible': 'success'
  }

  return (
    <div key={index} className="card ia-entry border-dark">
      <div className="card-body">
        <div className="ia-result-header" onClick={() => toggleEntry(index)}>
          <span className="badge badge-dark">Attack Path Verifier Report</span>
          <span className="ia-tool-name">Attack Path Validation</span>
          <span className="ia-toggle-hint">{isExpanded ? 'collapse' : 'expand'}</span>
        </div>

        {isExpanded && (
          <div style={{ marginTop: '10px' }}>
            {r.overall_verdict && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Overall Verdict</div>
                <span
                  className={`badge ia-severity-badge badge-${verdictClass[r.overall_verdict] || 'secondary'}`}
                >
                  {r.overall_verdict}
                </span>
              </div>
            )}

            {r.executive_summary && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Executive Summary</div>
                <p className="ia-assessment-body mb-0">{r.executive_summary}</p>
              </div>
            )}

            {r.hosts_compromised && r.hosts_compromised.length > 0 && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Hosts Compromised</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {r.hosts_compromised.map((host, i) => (
                    <span key={i} className="badge badge-danger">
                      {host}
                    </span>
                  ))}
                </div>
              </div>
            )}

            {r.attack_path_steps && r.attack_path_steps.length > 0 && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Attack Path Steps</div>
                {r.attack_path_steps.map((step, i) => (
                  <div key={i} className="card ia-attack-path mb-2">
                    <div className="card-body" style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <strong style={{ fontSize: '13px' }}>
                          {i + 1}. {step.step_name}
                        </strong>
                        <span
                          className={`badge badge-${step.success ? 'danger' : 'secondary'}`}
                          style={{ fontSize: '11px' }}
                        >
                          {step.success ? 'Exploited' : 'Failed'}
                        </span>
                      </div>
                      {step.step_description && (
                        <p className="ia-assessment-body mb-1" style={{ fontSize: '12px' }}>
                          {step.step_description}
                        </p>
                      )}
                      {step.target_host && (
                        <div style={{ fontSize: '12px', marginBottom: '4px' }}>
                          <strong>Target:</strong>{' '}
                          <span className="badge badge-secondary">{step.target_host}</span>
                        </div>
                      )}
                      {step.commands_executed && step.commands_executed.length > 0 && (
                        <div style={{ fontSize: '12px', marginBottom: '4px' }}>
                          <strong>Commands:</strong>
                          {step.commands_executed.map((cmd, j) => (
                            <pre
                              key={j}
                              style={{
                                background: '#1e1e1e',
                                color: '#d4d4d4',
                                padding: '4px 8px',
                                borderRadius: '3px',
                                fontSize: '11px',
                                marginBottom: '2px',
                                marginTop: '2px',
                                overflowX: 'auto'
                              }}
                            >
                              {cmd}
                            </pre>
                          ))}
                        </div>
                      )}
                      {step.command_outputs && step.command_outputs.length > 0 && (
                        <div style={{ fontSize: '12px', marginBottom: '4px' }}>
                          <strong>Outputs:</strong>
                          {step.command_outputs.map((out, j) => (
                            <pre
                              key={j}
                              style={{
                                background: '#f5f5f5',
                                padding: '4px 8px',
                                borderRadius: '3px',
                                fontSize: '11px',
                                marginBottom: '2px',
                                marginTop: '2px',
                                overflowX: 'auto',
                                maxHeight: '120px'
                              }}
                            >
                              {out}
                            </pre>
                          ))}
                        </div>
                      )}
                      {step.evidence && (
                        <div style={{ fontSize: '12px', marginBottom: '4px' }}>
                          <strong>Evidence:</strong> {step.evidence}
                        </div>
                      )}
                      {step.notes && (
                        <div style={{ fontSize: '12px', color: '#666' }}>
                          <strong>Notes:</strong> {step.notes}
                        </div>
                      )}
                    </div>
                  </div>
                ))}
              </div>
            )}

            {r.reproduction_commands && r.reproduction_commands.length > 0 && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Reproduction Commands (Attack Playbook)</div>
                <div
                  style={{
                    background: '#1e1e1e',
                    color: '#d4d4d4',
                    padding: '10px 12px',
                    borderRadius: '4px',
                    fontSize: '12px',
                    overflowX: 'auto'
                  }}
                >
                  {r.reproduction_commands.map((cmd, i) => (
                    <div key={i} style={{ marginBottom: '2px' }}>
                      <span style={{ color: '#666', marginRight: '8px' }}>{i + 1}.</span>
                      <code style={{ color: '#d4d4d4' }}>{cmd}</code>
                    </div>
                  ))}
                </div>
              </div>
            )}

            {r.defensive_recommendations && r.defensive_recommendations.length > 0 && (
              <div className="ia-assessment-section">
                <div className="ia-assessment-label">Defensive Recommendations</div>
                <ul
                  style={{
                    fontSize: '13px',
                    paddingLeft: '20px',
                    marginBottom: 0
                  }}
                >
                  {r.defensive_recommendations.map((rec, i) => (
                    <li key={i}>{rec}</li>
                  ))}
                </ul>
              </div>
            )}
          </div>
        )}
      </div>
    </div>
  )
}

export default AttackPathVerifierAgentReport
