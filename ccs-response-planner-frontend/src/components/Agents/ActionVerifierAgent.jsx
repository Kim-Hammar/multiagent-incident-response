...
import {
  API_EXAMPLES_URL,
  API_AGENTS_ACTION_VERIFIER_STEP_URL,
  API_AGENTS_ACTION_VERIFIER_TOOL_URL,
  API_AGENTS_ACTION_VERIFIER_PROMPT_URL,
  API_LLM_URL,
  API_AGENTS_REPORTS_URL
} from '../Common/constants'
import ActionVerifierAgentConfigTab from './ActionVerifierAgentConfigTab.jsx'
import ActionVerifierAgentReport from './ActionVerifierAgentReport.jsx'
...
/**
 * ActionVerifierAgent component — drives the action verifier agent loop with
 * human-in-the-loop tool approval.
 */
function ActionVerifierAgent() {
  ...
  const callStep = async (history, resumeJobId) => {
    ...
    try {
      ...
      if (!job_id) {
        const res = await fetch(API_AGENTS_ACTION_VERIFIER_STEP_URL, {
          method: 'POST',
          headers: {
      ...
      await pollJobEvents({
        ...
        onEvent: (event) => {
          ...
          } else if (event.type === 'action_verification') {
            finalEntry = {
              role: 'model',
              type: 'action_verification',
              action_verification: event.action_verification,
              thinking_trace: event.thinking_trace || ''
            }
          } else if (
      ...
      if (finalEntry) {
        ...
        if (finalEntry.type === 'action_verification') {
          saveReport(finalEntry.action_verification, updated)
        }
      } else if (accumulated) {
        let report
        try {
        ...
        const fallbackHistory = [
          ...
          ...compactionEntries,
          ...dtEntries,
          { role: 'model', type: 'action_verification', action_verification: report }
        ]
  ...
  const handleApprove = async () => {
    ...
    if (STREAMING_TOOLS.has(proposal.tool_name)) {
      ...
      try {
        const { result } = await executeStreamingTool({
          url: API_AGENTS_ACTION_VERIFIER_TOOL_URL,
    ...
    try {
      const res = await fetch(API_AGENTS_ACTION_VERIFIER_TOOL_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const getPromptText = async () => {
    const res = await fetch(API_AGENTS_ACTION_VERIFIER_PROMPT_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res = await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=action-verifier`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (res.ok) {
  ...
  const deleteAllReports = async () => {
    try {
      await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=action-verifier`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      await fetchHistory()
    ...
    <div>
      ...
      {activeTab === 'agents' && (
        <AgentConfigTable
          ...
          rows={[
            {
              label: 'Action Verifier Agent',
              model: selectedModel,
              setModel: setSelectedModel,
              promptUrl: API_AGENTS_ACTION_VERIFIER_PROMPT_URL,
              iteration: null,
      ...
      {activeTab === 'history' && (
        <AgentHistoryTab
          ...
          renderReport={(report) => (
            <ActionVerifierAgentReport
              entry={{ type: 'action_verification', action_verification: report }}
              index="history"
              isExpanded={true}
              toggleEntry={() => {}}
            />
...
export default ActionVerifierAgent
