...
import {
  API_EXAMPLES_URL,
  API_AGENTS_ACTION_VALIDATOR_STEP_URL,
  API_AGENTS_ACTION_VALIDATOR_TOOL_URL,
  API_AGENTS_ACTION_VALIDATOR_PROMPT_URL,
  API_LLM_URL,
  API_AGENTS_REPORTS_URL
} from '../Common/constants'
import ActionValidatorAgentConfigTab from './ActionValidatorAgentConfigTab.jsx'
import ActionValidatorAgentReport from './ActionValidatorAgentReport.jsx'
...
/**
 * ActionValidatorAgent component — drives the action validator agent loop with
 * human-in-the-loop tool approval.
 */
function ActionValidatorAgent() {
  ...
  const callStep = async (history, resumeJobId) => {
    ...
    try {
      ...
      if (!job_id) {
        const res = await fetch(API_AGENTS_ACTION_VALIDATOR_STEP_URL, {
          method: 'POST',
          headers: {
      ...
      await pollJobEvents({
        ...
        onEvent: (event) => {
          ...
          } else if (event.type === 'action_validation') {
            finalEntry = {
              role: 'model',
      ...
      if (finalEntry) {
        ...
        if (finalEntry.type === 'action_validation') {
          saveReport(finalEntry.action_validation, updated)
        }
      } else if (accumulated) {
        let report
        try {
          ...
          report = {
            action_name: 'Unknown',
            action_description: accumulated,
            commands_executed: [],
            command_results: [],
        ...
        const fallbackHistory = [
          ...
          ...compactionEntries,
          ...dtEntries,
          { role: 'model', type: 'action_validation', action_validation: report }
        ]
  ...
  const handleApprove = async () => {
    ...
    if (STREAMING_TOOLS.has(proposal.tool_name)) {
      ...
      try {
        const { result } = await executeStreamingTool({
          url: API_AGENTS_ACTION_VALIDATOR_TOOL_URL,
    ...
    try {
      const res = await fetch(API_AGENTS_ACTION_VALIDATOR_TOOL_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const getPromptText = async () => {
    const res = await fetch(API_AGENTS_ACTION_VALIDATOR_PROMPT_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=action-validator`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (res.ok) {
  ...
  const deleteAllReports = async () => {
    try {
      await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=action-validator`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      await fetchHistory()
    ...
    <div>
      ...
      {activeTab === 'configuration' && (
        <div style={{ marginTop: '16px' }}>
          <div className="form-check">
            ...
            />
            <label className="form-check-label" htmlFor="av-dt-enabled">
      ...
      {activeTab === 'agents' && (
        <AgentConfigTable
          ...
          rows={[
            {
              label: 'Action Validator Agent',
              model: selectedModel,
              setModel: setSelectedModel,
              promptUrl: API_AGENTS_ACTION_VALIDATOR_PROMPT_URL,
              iteration: null,
      ...
      {activeTab === 'history' && (
        <AgentHistoryTab
          ...
          renderReport={(report) => (
            <ActionValidatorAgentReport
              entry={{ type: 'action_validation', action_validation: report }}
              index="history"
              isExpanded={true}
              toggleEntry={() => {}}
            />
...
export default ActionValidatorAgent
