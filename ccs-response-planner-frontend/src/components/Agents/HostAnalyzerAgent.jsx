...
import {
  API_EXAMPLES_URL,
  API_AGENTS_HOST_ANALYZER_STEP_URL,
  API_AGENTS_HOST_ANALYZER_TOOL_URL,
  API_AGENTS_HOST_ANALYZER_PROMPT_URL,
  API_LLM_URL,
  API_AGENTS_REPORTS_URL
} from '../Common/constants'
import HostAnalyzerAgentConfigTab from './HostAnalyzerAgentConfigTab.jsx'
import HostAnalyzerAgentReport from './HostAnalyzerAgentReport.jsx'
...
/**
 * HostAnalyzerAgent component — drives the host analyzer agent loop with
 * human-in-the-loop tool approval.
 */
function HostAnalyzerAgent() {
  ...
  const {
    ...
    onRestore: (session) => {
      ...
      setHostDescription(inputs.hostDescription || '')
  ...
  const callStep = async (history, resumeJobId) => {
    ...
    try {
      ...
      if (!job_id) {
        const res = await fetch(API_AGENTS_HOST_ANALYZER_STEP_URL, {
          method: 'POST',
          headers: {
      ...
      await pollJobEvents({
        ...
        onEvent: (event) => {
          ...
          } else if (event.type === 'host_analysis') {
            finalEntry = {
              role: 'model',
              type: 'host_analysis',
      ...
      if (finalEntry) {
        ...
        if (finalEntry.type === 'host_analysis') {
          saveReport(finalEntry.host_analysis, updated)
        }
      } else if (accumulated) {
        let report
        try {
          ...
          report = {
            ...
            compromise_details: accumulated,
            attack_vectors: [],
            security_posture: '',
  ...
  const handleApprove = async () => {
    ...
    if (STREAMING_TOOLS.has(proposal.tool_name)) {
      ...
      try {
        const { result } = await executeStreamingTool({
          url: API_AGENTS_HOST_ANALYZER_TOOL_URL,
    ...
    try {
      const res = await fetch(API_AGENTS_HOST_ANALYZER_TOOL_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const getPromptText = async () => {
    const res = await fetch(API_AGENTS_HOST_ANALYZER_PROMPT_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=host-analyzer`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (res.ok) {
  ...
  const deleteAllReports = async () => {
    try {
      await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=host-analyzer`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      await fetchHistory()
    ...
    <div>
      ...
      {activeTab === 'agents' && (
        <AgentConfigTable
          ...
          rows={[
            {
              label: 'Host Analyzer Agent',
              model: selectedModel,
              setModel: setSelectedModel,
              promptUrl: API_AGENTS_HOST_ANALYZER_PROMPT_URL,
              iteration: null,
      ...
      {activeTab === 'history' && (
        <AgentHistoryTab
          ...
          renderReport={(report) => (
            <HostAnalyzerAgentReport
              entry={{ type: 'host_analysis', host_analysis: report }}
              index="history"
              isExpanded={true}
              toggleEntry={() => {}}
            />
...
export default HostAnalyzerAgent
