...
import ReportVerifierConfigTab from './ReportVerifierConfigTab.jsx'
import ReportVerifierReport from './ReportVerifierReport.jsx'
...
/**
 * ReportVerifierAgent component — drives the report review agent loop with
 * human-in-the-loop tool approval. Renders 3 inner tabs.
 */
function ReportVerifierAgent() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res = await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=report_verifier`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (res.ok) {
  ...
  const deleteAllReports = async () => {
    try {
      await fetch(`${API_AGENTS_REPORTS_URL}?agent_type=report_verifier`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      await fetchHistory()
...
export default ReportVerifierAgent
