...
import {
  API_EXAMPLES_URL,
  API_AGENTS_PLAN_VERIFIER_STEP_URL,
  API_AGENTS_PLAN_VERIFIER_TOOL_URL,
  API_AGENTS_PLAN_VERIFIER_PROMPT_URL,
  API_LLM_URL,
  API_AGENTS_REPORTS_URL,
  API_DT_PYTHON_STOP_URL
} from '../Common/constants'
import PlanVerifierAgentConfigTab from './PlanVerifierAgentConfigTab.jsx'
...
/**
 * PlanVerifierAgent component — drives the validation agent loop with
 * human-in-the-loop tool approval. Renders 3 inner tabs.
 */
function PlanVerifierAgent() {
  ...
  const callStep = async (history, resumeJobId) => {
    ...
    try {
      ...
      if (!job_id) {
        const res = await fetch(API_AGENTS_PLAN_VERIFIER_STEP_URL, {
          method: 'POST',
          headers: {
      ...
      await pollJobEvents({
        ...
        onEvent: (event) => {
          ...
          } else if (event.type === 'plan_verifier_report') {
            finalEntry = {
              role: 'model',
              type: 'plan_verifier_report',
              plan_verifier_report: event.plan_verifier_report,
              thinking_trace: event.thinking_trace || ''
            }
          } else if (
      ...
      if (finalEntry) {
        ...
        if (finalEntry.type === 'plan_verifier_report') {
          saveReport(finalEntry.plan_verifier_report, updated)
        }
      } else if (accumulated) {
        let report
        try {
        ...
        const fallbackHistory = [
          ...
          ...compactionEntries,
          ...dtEntries,
          { role: 'model', type: 'plan_verifier_report', plan_verifier_report: report }
        ]
  ...
  const handleApprove = async () => {
    ...
    if (STREAMING_TOOLS.has(proposal.tool_name)) {
      const isActionVerifiers = proposal.tool_name === 'run_action_verifiers'
      const streamEntry = {
        type: 'tool_streaming',
        tool_name: proposal.tool_name,
        output: '',
      ...
      try {
        const { result } = await executeStreamingTool({
          url: API_AGENTS_PLAN_VERIFIER_TOOL_URL,
    ...
    try {
      const res = await fetch(API_AGENTS_PLAN_VERIFIER_TOOL_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const resumeToolJob = async (jobId, toolName, startTime) => {
    const isActionVerifiers = toolName === 'run_action_verifiers'
    const streamEntry = {
      type: 'tool_streaming',
      tool_name: toolName,
      output: '',
    ...
    try {
      ...
      let updated
      setConversationHistory((prev) => {
        const sEntry = prev.find((e) => e.type === 'tool_streaming' && e.tool_name === toolName)
        if (sEntry) {
  ...
  const getPromptText = async () => {
    const res = await fetch(API_AGENTS_PLAN_VERIFIER_PROMPT_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
    ...
    <div>
      ...
      {activeTab === 'history' && (
        <AgentHistoryTab
          ...
          renderReport={(report) => (
            <PlanVerifierAgentReport
              entry={{ type: 'plan_verifier_report', plan_verifier_report: report }}
              index="history"
              isExpanded={true}
              toggleEntry={() => {}}
            />
...
export default PlanVerifierAgent
