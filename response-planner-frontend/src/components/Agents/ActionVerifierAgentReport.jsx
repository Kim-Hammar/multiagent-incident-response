/**
 * Renders the final action verification report for the Action Verifier Agent.
 */
function ActionVerifierAgentReport({ entry, index, isExpanded, toggleEntry }) {
  if (entry.type !== 'action_verification') return null

  const r = entry.action_verification || {}
  const outcomeClass = {
    ...
    <div key={index} className="card ia-entry border-dark">
      <div className="card-body">
        <div className="ia-result-header" onClick={() => toggleEntry(index)}>
          ...
          <span className="ia-tool-name">{r.action_name || 'Action Verification Report'}</span>
          {r.outcome && (
            <span
...
export default ActionVerifierAgentReport
