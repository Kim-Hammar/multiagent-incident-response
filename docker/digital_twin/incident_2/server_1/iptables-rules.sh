...
iptables -A FORWARD -s 10.1.0.0/24 -d 10.0.1.10 -p tcp --dport 80 -j ACCEPT
iptables -A FORWARD -s 10.1.0.0/24 -d 10.0.1.10 -p tcp --dport 443 -j ACCEPT
iptables -A FORWARD -s 10.1.0.0/24 -d 10.0.1.10 -p tcp --dport 8080 -j ACCEPT

# Allow SSH from Internet to Server 3 (jump host)
iptables -A FORWARD -s 10.1.0.0/24 -d 10.0.1.20 -p tcp --dport 22 -j ACCEPT
...
iptables -A FORWARD -s 10.0.1.0/24 -d 10.0.1.0/24 -p icmp -j ACCEPT
iptables -A FORWARD -s 10.0.2.0/24 -d 10.0.2.0/24 -p icmp -j ACCEPT

# Allow ICMP from Internet to DMZ (for reachability tests)
iptables -A FORWARD -s 10.1.0.0/24 -d 10.0.1.0/24 -p icmp -j ACCEPT

# Block DMZ hosts from reaching LAN hosts they shouldn't
# (Server 2 can only reach Server 4, not Server 5/6)
iptables -A FORWARD -s 10.0.1.10 -d 10.0.2.50 -j DROP
iptables -A FORWARD -s 10.0.1.10 -d 10.0.2.60 -j DROP

# Block LAN outbound to Internet
iptables -A FORWARD -s 10.0.2.0/24 -d 10.1.0.0/24 -j DROP
