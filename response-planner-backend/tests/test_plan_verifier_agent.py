"""Tests for the /api/agents/plan-verifier endpoints."""
import json
import time
...
def test_plan_verifier_step_401_without_token(
    client: FlaskClient,
) -> None:
    """
    POST /api/agents/plan-verifier/step without a token returns 401.
    """
    resp = client.post(
        "/api/agents/plan-verifier/step",
        data=json.dumps({
...
def test_plan_verifier_step_400_missing_fields(
    client: FlaskClient, auth_headers: dict[str, str],
) -> None:
    """
    POST /api/agents/plan-verifier/step with no fields returns 400.
    """
    resp = client.post(
        "/api/agents/plan-verifier/step",
        data=json.dumps({}),
...
@patch(
    "response_planner_backend.rest_api.resources.agents"
...
def test_plan_verifier_step_streams_events(
    ...
    """
    POST /api/agents/plan-verifier/step streams text then
    tool_proposal.
    """
...
def test_plan_verifier_tool_400_unknown_tool(
    client: FlaskClient, auth_headers: dict[str, str],
) -> None:
    """
    POST /api/agents/plan-verifier/tool with unknown tool returns 400.
    """
    resp = client.post(
...
def test_plan_verifier_tool_executes_dt_exec(
    mock_agent_cls: MagicMock,
    client: FlaskClient,
    auth_headers: dict[str, str],
) -> None:
    """
    POST /api/agents/plan-verifier/tool runs dt_exec as background job.
    """
    mock_agent = MagicMock()
...
def test_plan_verifier_step_accepts_planner_report_id(
    mock_db: MagicMock,
    mock_agent_cls: MagicMock,
    _mock_redeploy: MagicMock,
    _mock_sandbox: MagicMock,
    ...
    """
    POST /api/agents/plan-verifier/step with planner_report_id
...
def test_plan_verifier_prompt_renders(
    client: FlaskClient, auth_headers: dict[str, str],
) -> None:
    """
    POST /api/agents/plan-verifier/prompt renders the system prompt.
    """
    resp = client.post(
        "/api/agents/plan-verifier/prompt",
        data=json.dumps({
