...
from response_planner_backend.rest_api.rest_api import create_app
...
def mock_db() -> Generator[MagicMock, None, None]:
    ...
    """
    with patch(
        "response_planner_backend.rest_api.util.auth.DatabaseFacade"
    ) as auth_mock, patch(
        "response_planner_backend.rest_api.resources.login.routes"
        ".DatabaseFacade"
    ) as login_mock, patch(
        ...
        "response_planner_backend.rest_api.resources.example"
        ".routes.DatabaseFacade"
    ) as example_db_mock:
        for mock in (
