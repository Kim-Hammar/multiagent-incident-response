"""Tests for the ReportVerifierAgent."""
import base64
import json
...
def test_tool_dispatch_has_all_tools() -> None:
    ...
    because the verifier should not regenerate images.
    """
    expected = {
        "tavily_search",
        "nvd_search",
...
@patch(
    "response_planner_backend.agents.report_verifier_agent"
    ".agent.genai",
)
def test_agent_step_stream_yields_tool_proposal(
    ...
    """
    ReportVerifierAgent.step_stream should yield text then
...
def test_agent_step_stream_yields_report_review(
    ...
    """
    ReportVerifierAgent.step_stream should yield a report_review
...
def test_agent_step_stream_yields_thinking_events(
    ...
    """
    ReportVerifierAgent.step_stream should yield thinking events
...
def test_format_incident_report_with_full_data() -> None:
    ...
    formatted = ReportVerifierAgent._format_incident_report(
        report,
    )
...
def test_format_iteration_note_first() -> None:
    ...
    note = ReportVerifierAgent._format_iteration_note(1)
    assert note == ""
...
def test_format_iteration_note_second() -> None:
    ...
    note = ReportVerifierAgent._format_iteration_note(2)
...
def test_format_iteration_note_fourth() -> None:
    ...
    note = ReportVerifierAgent._format_iteration_note(4)
...
def test_serialize_part_with_thought_signature() -> None:
    ...
    serialized = ReportVerifierAgent._serialize_part(part)
...
def test_decode_raw_parts_with_thought_signature() -> None:
    ...
    decoded = ReportVerifierAgent._decode_raw_parts(raw)
...
def test_has_used_tool_with_tool_result() -> None:
    ...
    assert ReportVerifierAgent._has_used_tool(history) is True
...
def test_has_used_tool_without_tool_result() -> None:
    ...
    assert ReportVerifierAgent._has_used_tool(history) is False
...
@patch(
    "response_planner_backend.agents.report_agent"
    ".tools.TavilyClient",
)
