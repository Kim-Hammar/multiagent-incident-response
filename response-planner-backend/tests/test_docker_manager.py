...
DOCKER_MODULE = (
    "response_planner_backend.docker_manager.docker_manager.docker"
)
...
class TestDeploy:
    ...
    def test_deploy_creates_networks_and_containers(
        ...
        assert result["networks"][0] == "dt_net_zone1"
    ...
    def test_deploy_reuses_existing_network(
        ...
        assert "dt_net_zone1" in result["networks"]
    ...
    def test_deploy_connects_multi_network_host(
        ...
        client.containers.get.side_effect = NotFound("not found")
        container = MagicMock()
        container.name = "dt_i1_log_collector"
        container.status = "running"
        client.containers.create.return_value = container
...
class TestStop:
    ...
    def test_stop_removes_containers_and_networks(
        ...
        """
        Stop should remove all dt_ containers and networks.
        """
        client = MagicMock()
        mock_docker.from_env.return_value = client
        c1 = MagicMock()
        c1.name = "dt_i1_server_1"
        c2 = MagicMock()
        c2.name = "dt_i1_server_2"
        client.containers.list.return_value = [c1, c2]
        n1 = MagicMock()
        n1.name = "dt_net_zone1"
        n2 = MagicMock()
        n2.name = "dt_net_zone2"
        other = MagicMock()
        other.name = "bridge"
    ...
    def test_stop_emits_progress_messages(
        ...
        messages = [i["message"] for i in items
                    if i["type"] == "progress"]
        assert any("Removing dt_i1_server_1" in m for m in messages)
        assert any("Removed dt_i1_server_1" in m for m in messages)
...
class TestStatus:
    ...
    def test_status_returns_deployed_true(
        ...
        assert len(result["containers"]) == 1
        assert result["networks"] == ["dt_net_zone1"]
...
class TestExec:
    ...
    def test_exec_create_returns_raw_socket_and_id(
        ...
        result_sock, result_id = DockerManager.exec_create("dt_s1")
...
class TestConfigIdScoping:
    ...
    def test_deploy_uses_config_id_in_network_name(
        ...
        result = [i for i in items
                  if i["type"] == "result"][0]["data"]
        assert result["networks"][0] == "dt_net_42_zone1"
    ...
    def test_stop_scoped_by_config(
        ...
        c1.name = "dt_i1_server_1"
        c2 = MagicMock()
        c2.name = "dt_i2_server_1"
        client.containers.list.return_value = [c1, c2]
        n1 = MagicMock()
        n1.name = "dt_net_1_zone1"
        n2 = MagicMock()
        n2.name = "dt_net_2_internet"
        ...
        assert result["removed"][0] == "dt_i1_server_1"
        c1.remove.assert_called_once_with(force=True)
    ...
    def test_status_scoped_by_config(
        ...
        assert result["networks"] == ["dt_net_1_zone1"]
