# Response Planner Backend

[![PyPI version](https://badge.fury.io/py/response-planner-backend.svg)](https://badge.fury.io/py/response-planner-backend)
![PyPI - Downloads](https://img.shields.io/pypi/dm/response-planner-backend)

Python backend for the Incident Response Planner. Provides a Flask REST API, a multi-agent system for incident response planning, external security tool integrations, and a digital twin manager.

## Architecture

The backend source lives under `src/response_planner_backend/`:
...
docker compose exec app bash -c "cd response-planner-backend && pytest --cov=response_planner_backend"
docker compose exec app bash -c "cd response-planner-backend && flake8 src tests"
docker compose exec app bash -c "cd response-planner-backend && mypy src"
```
