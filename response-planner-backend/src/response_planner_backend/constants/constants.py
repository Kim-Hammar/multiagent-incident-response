"""
Shared constants for the Response Planner backend.
"""
import base64
...
class DB:
    ...
    DEFAULT_PORT = 5432
    DEFAULT_DB_NAME = "incident_response"
    DEFAULT_USER = "incident_response"
...
class GENERAL:
    ...
    General constants
    """
    APP_NAME = "Incident Response Planner"
...
class DOCKER:
    ...
    """
    NETWORK_PREFIX = "dt_net_"
    CONTAINER_PREFIX = "dt_"
    PYTHON_SANDBOX_IMAGE = "dt-python-sandbox:latest"
    PYTHON_SANDBOX_CONTAINER = "python_sandbox"
    ATTACKER_IMAGE = "dt-attacker:latest"
        ...
        "hosts": [
            {
                ...
                "docker_image": "dt-attacker:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-gateway:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-firewall:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-logcollector:latest",
                "ip_addresses": {
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server1:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server2:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server3:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server4:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server5:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i1-server6:latest",
    ...
    INCIDENT_2_CONFIG: dict[str, Any] = {
        ...
        "hosts": [
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server1:latest",
                "ip_addresses": {
                    "internet": "10.1.0.1",
                    "dmz": "10.1.1.1",
                    "lan": "10.1.2.1",
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server2:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server3:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server4:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server5:latest",
            ...
            },
            {
                ...
                "docker_image": "dt-i2-server6:latest",
...
class EXAMPLES:
    ...
    SYSTEM_DESCRIPTION = (
        ...
        "PHP-FPM 7.4 customer portal (pool runs as root — "
        "restart requires changing user/group to www-data in "
        "/etc/php/7.4/fpm/pool.d/www.conf and fixing log "
        "ownership before restarting the service), "
        "SQLite database backing the portal "
        "(at /var/www/html/portal.db), "
        "dnsmasq internal DNS\n"
        "Server 2 (10.0.2.2, Debian 11): vsftpd FTP, "
        "cron nightly backups\n"
        "Server 3 (10.0.3.3, Ubuntu 20): OpenSSH, "
        "cron CI/CD build pipeline\n"
        "Server 4 (10.0.3.4, Debian 11): Postfix SMTP mail "
        "server\n"
        "Server 5 (10.0.4.5, Debian 11): SSH, Python REST API, "
        "Redis session cache\n"
        "Server 6 (10.0.4.6, Debian 8): PostgreSQL 9.4 "
        "database, Samba 4.2.14 file shares"
    )
    ...
    ATTACK_PATH = (
        "1. Initial access — SSH brute-force into Server 3: "
        "From the attacker host (192.168.1.50), use Hydra "
        "(or similar) to brute-force SSH on Server 3 "
        "(10.0.3.3:22). The firewall only forwards traffic "
        "to Server 2 and Server 3, so Server 3 SSH is "
        "reachable from the perimeter. Try common "
        "username/password lists (e.g. root:root, "
        "admin:admin, ubuntu:ubuntu). Once a valid "
        "credential is found, SSH in to obtain a shell on "
        "Server 3.\n"
        "2. Lateral movement — SambaCry exploit to Server 6: "
        "From the compromised Server 3, target Server 6 "
        "(10.0.4.6) which is reachable via the cross-zone "
        "route through the Log Collector (10.0.3.252 → "
        "10.0.4.252). Server 6 runs Debian 8 with an "
        "outdated Samba installation vulnerable to "
        "CVE-2017-7494 (SambaCry / is_known_pipename). "
        "Server 6 exposes a writable Samba share named "
        "'public' (mapped to /srv/public on the host "
        "filesystem, guest access enabled). "
        "This vulnerability allows unauthenticated remote "
        "code execution: upload a malicious shared library "
        "(.so) to the 'public' share via smbclient "
        "(e.g. smbclient //10.0.4.6/public -N -c "
        "'put payload.so'), then trigger execution by "
        "opening a named pipe with the absolute filesystem "
        "path of the .so (e.g. /srv/public/payload.so) via "
        "the IPC$ share. Samba's smbd calls dlopen() on "
        "the path, executing the payload as root. Use "
        "Metasploit (exploit/linux/samba/is_known_pipename) "
        "or a standalone Python exploit using Impacket's "
        "SMBConnection to trigger the pipe.\n"
        "3. SQL injection — data exfiltration from Server 1: "
        "From the compromised Server 6, target the PHP "
        "customer portal on Server 1 (10.0.2.1:80) which "
        "is reachable via the cross-zone adjacency link "
        "S1-S6. The PHP-FPM application on Server 1 has a "
        "SQL injection vulnerability in a query parameter. "
        "Craft a UNION SELECT payload "
        "(e.g. ' UNION SELECT username,password FROM "
        "users--) to extract data from the SQLite "
        "database backing the portal. Use curl from "
        "Server 6 to send the injection via HTTP GET/POST "
        "to the vulnerable endpoint on port 80. The "
...
class EXAMPLES_2:
    ...
    ATTACK_PATH = (
        "1. Initial access — Tomcat deserialization RCE "
        "(CVE-2020-9484): From the attacker host "
        "(198.51.100.45), exploit the Apache Tomcat 9.0.30 "
        "session persistence deserialization vulnerability "
        "on Server 2 (10.1.1.10:8080). The firewall allows "
        "HTTP/HTTPS/8080 from the Internet to the DMZ. "
        "Craft a malicious serialized Java object and send "
        "it via a POST request to /api/v1/upload, abusing "
        "the FileStore session manager to trigger "
        "deserialization of a gadget chain (e.g. "
        "CommonsCollections or URLDNS) that executes "
        "arbitrary commands as the Tomcat user.\n"
        "2. Persistence — JSP webshell and cron backdoor: "
        "Using the RCE obtained in step 1, write a JSP "
        "webshell to the Tomcat webapps directory at "
        "/shell.jsp (e.g. via echo or curl from the "
        "attacker's HTTP server). Verify access by "
        "requesting GET /shell.jsp from the attacker. "
        "Then install a cron-based persistence mechanism "
        "by writing a reverse-shell script to "
        "/etc/cron.d/.cleanup.sh that connects back to "
        "the attacker on a set interval, ensuring "
        "continued access even if the webshell is "
        "removed.\n"
        "3. Credential theft — harvest database credentials: "
        "On Server 2, read the Tomcat configuration file "
        "/opt/tomcat/conf/db_config.xml which contains "
        "plaintext PostgreSQL credentials: username "
        "'app_svc' with password 'SuperSecret123!'. These "
        "credentials are used by the Java web application "
        "to connect to the CRM database on Server 4.\n"
        "4. Lateral movement — PostgreSQL COPY FROM PROGRAM "
        "(CVE-2019-9193): From Server 2, connect to "
        "PostgreSQL on Server 4 (10.1.2.10:5432) using "
        "the stolen credentials (app_svc / SuperSecret123! "
        "on database crm_production). The firewall allows "
        "port 5432 from Server 2 to Server 4. Once "
        "connected, exploit the COPY FROM PROGRAM feature "
        "to execute OS commands as the postgres user: "
        "first run CREATE TABLE cmd_exec(cmd_output text) "
        "to create a staging table, then COPY cmd_exec "
        "FROM PROGRAM '<command>' to execute arbitrary "
        "shell commands (e.g. id, whoami, download a "
        "payload). This gives full command execution on "
        "Server 4.\n"
        "5. Impact — crypto-miner deployment and data "
        "exfiltration: Using the command execution from "
        "step 4, download and deploy the xmrig crypto-miner "
        "binary to /tmp/.kworker on Server 4 (disguised "
        "as a kernel worker process, PID 31337). Configure "
        "it to mine to the attacker's wallet. Additionally, "
        "set up DNS tunneling for data exfiltration by "
        "encoding stolen data (e.g. credentials, database "
        "dumps) as base64 subdomains in TXT queries to "
        "evil.example.com (e.g. "
        "aHR0cDovL2V4ZmlsLmV4YW1wbGUu.t.evil.example.com). "
        "The DNS queries pass through Server 5's dnsmasq "
