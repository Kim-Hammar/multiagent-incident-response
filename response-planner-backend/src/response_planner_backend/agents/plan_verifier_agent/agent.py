"""
PlanVerifierAgent — uses Gemini with function calling to verify
...
from response_planner_backend.agents.stream_timeout import (
    iter_with_idle_timeout,
)
from response_planner_backend.agents.anthropic_adapter import (
...
from response_planner_backend.agents.plan_verifier_agent.tool_declarations import (
    TOOL_DECLARATIONS,
...
REPORT_TOOL_NAME = "produce_plan_verifier_report"
...
class PlanVerifierAgent:
    ...
    def _format_verification_feedback(
        ...
        """
        Format a previous plan verifier report into a concise
        feedback section for the system prompt.

        :param report: the previous plan_verifier_report dict
    ...
    def step_stream(
        ...
        - ``{"type": "plan_verifier_report", ...}`` for a final report
        ...
        system_prompt = build_system_prompt(
            ...
            verification_feedback=(
                self._format_verification_feedback(
        ...
        if is_anthropic_model(effective_model):
            if has_policy:
                initial_text = (
                    "Please validate the response "
                    "plan using the trained RL "
                    "policy. Start by assessing "
                    "the initial digital twin "
                    "state, then query the policy "
                    "for each action."
                )
            else:
                initial_text = (
                    "Please validate the response "
                    "plan by applying actions "
                    "sequentially on the digital "
                    "twin and checking recovery "
                    "and service state after "
                    "each action."
                )
        ...
        if function_call:
            if function_call.name == REPORT_TOOL_NAME:
                event: dict[str, Any] = {
                    ...
                    "plan_verifier_report": self._normalize_args(
            ...
            yield self._parse_plan_verifier_report(full_text)
    ...
    def _normalize_args(obj: Any) -> Any:
        ...
        if hasattr(obj, "items"):
            return {
                str(k): PlanVerifierAgent._normalize_args(v)
                for k, v in obj.items()
            }
        if hasattr(obj, "__iter__"):
            return [
                PlanVerifierAgent._normalize_args(v)
                for v in obj
            ]
        return obj
    ...
    def _parse_plan_verifier_report(
        self, text: str,
    ) -> dict[str, Any]:
        """
        ...
        :return: a dict with type plan_verifier_report and fields
        """
        cleaned = re.sub(
            r"^```(?:json)?\s*\n?", "", text.strip(),
        )
    ...
    def _build_contents(
        ...
        for _idx, entry in enumerate(history):
            ...
            elif entry_type == "plan_verifier_report":
                report = entry.get(
                    "plan_verifier_report", {},
                )
