...
from response_planner_backend.db.database_facade import (
    DatabaseFacade,
)
...
_FINAL_REPORT_TYPES = {"action_verification"}
...
def _run_single_action_verifier(
    action: dict[str, Any],
    ...
    """
    Run an ActionVerifierAgent for a single action.
    ...
    """
    from response_planner_backend.agents.action_verifier_agent.agent import (  # noqa: E501
        ActionVerifierAgent,
    )
    ...
    try:
        ...
        for step_num in range(_MAX_INNER_STEPS):
                ...
                try:
                    for event in agent.step_stream(
                        **av_kwargs,
                        ...
                        elif etype == "action_verification":
                            action_verification = (
                                event.get(
                                    "action_verification",
                                    {},
            ...
            if action_verification is not None:
                break
        ...
        try:
            ...
            logger.warning(
                "Failed to save action-verifier "
...
def run_action_verifiers_stream(
    actions: list[dict[str, Any]],
    context: dict[str, Any] | None = None,
) -> Generator[dict[str, Any], None, None]:
    """
    Run parallel ActionVerifierAgents for multiple actions.

    Spawns one thread per action, each running an
    ActionVerifierAgent to completion with auto-approved
    ...
    for action in actions:
        t = threading.Thread(
            target=_run_single_action_verifier,
    ...
    done_count = 0
    total = len(actions)
    action_verifications: dict[str, Any] = {}
    ...
    while done_count < total:
        ...
        if event.get("type") == "_agent_done":
            ...
            action_verifications[agent_id] = event.get(
                "action_verification", {},
            )
            yield {
                "type": "sub_event",
...
STREAMING_TOOL_DISPATCH: dict[
    ...
    "run_action_verifiers": run_action_verifiers_stream,
}
