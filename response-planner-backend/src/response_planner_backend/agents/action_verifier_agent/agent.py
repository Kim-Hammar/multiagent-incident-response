"""
ActionVerifierAgent — uses Gemini with function calling to validate
...
from response_planner_backend.agents.action_verifier_agent.tool_declarations import (
    TOOL_DECLARATIONS,
)
...
REPORT_TOOL_NAME = "produce_action_verification"
...
class ActionVerifierAgent:
    ...
    def step_stream(
        ...
        - ``{"type": "action_verification", ...}`` for a final report
        ...
        if is_anthropic_model(effective_model):
            for ev in anthropic_stream_step(
                ...
                final_event_type="action_verification",
                thinking_budget=THINKING_BUDGET,
                images=(
                    images
        ...
        if function_call:
            ...
            yield self._parse_action_verification(
                full_text,
            )
    ...
    def _normalize_args(obj: Any) -> Any:
        ...
        if hasattr(obj, "items"):
            return {
                str(k): (
                    ActionVerifierAgent._normalize_args(v)
                )
    ...
    def _parse_action_verification(
        self, text: str,
    ) -> dict[str, Any]:
        """
        Parse LLM text output into a structured action verification.
        ...
        :return: a dict with type action_verification and fields
        """
        cleaned = re.sub(
            r"^```(?:json)?\s*\n?", "", text.strip(),
        )
    ...
    def _build_contents(
        ...
        for _idx, entry in enumerate(history):
            ...
            elif entry_type == "action_verification":
