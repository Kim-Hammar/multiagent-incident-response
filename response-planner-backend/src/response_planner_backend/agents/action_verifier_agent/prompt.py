"""
System prompt template for the ActionVerifierAgent.

The ActionVerifierAgent validates a single specific action from a
...
side-effect.
4. Call `produce_action_verification` with your findings.
...
- **produce_action_verification**: Call this ONLY after executing the \
...
`produce_action_verification` with what you have.
"""


def build_system_prompt(**kwargs: str) -> str:
    """
    Render the action verifier system prompt with the given context.
