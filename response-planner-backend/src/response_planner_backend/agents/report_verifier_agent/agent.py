"""
ReportVerifierAgent — uses Gemini with function calling to verify
...
from response_planner_backend.agents.report_verifier_agent.tool_declarations import (
    ALL_DECLARATIONS,
...
class ReportVerifierAgent:
    """
    An agent that uses Gemini function calling to verify
    an incident report and produce a structured verification.
    """
    ...
    def step_stream(
        ...
        logger.info(
            "ReportVerifierAgent calling Gemini "
            "model=%s turns=%d",
        ...
        logger.info(
            "ReportVerifierAgent stream done: "
    ...
    def _normalize_args(obj: Any) -> Any:
        ...
        if hasattr(obj, "items"):
            return {
                str(k): (
                    ReportVerifierAgent._normalize_args(v)
                )
