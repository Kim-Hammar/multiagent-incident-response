"""
HostAnalyzerAgent — uses Gemini with function calling to analyze
a single host within an incident and produce a structured report.
"""
import base64
import json
import logging
...
from response_planner_backend.agents.host_analyzer_agent.tool_declarations import (
    TOOL_DECLARATIONS,
)
...
REPORT_TOOL_NAME = "produce_host_analysis"
...
def _build_initial_message(
    ...
    """
    text = (
        "Please analyze the specified host. "
        "Use multiple tools to gather comprehensive "
        "information about this host before producing "
        "your final analysis."
    )
...
class HostAnalyzerAgent:
    """
    An agent that uses Gemini function calling to invoke
    security information tools and produce a structured
    analysis of a single host.
    """
    ...
    def step_stream(
        ...
        - ``{"type": "host_analysis", ...}`` for a final report
        ...
        if is_anthropic_model(effective_model):
            for ev in anthropic_stream_step(
                ...
                final_event_type="host_analysis",
                thinking_budget=THINKING_BUDGET,
                images=(
                    images
        ...
        if function_call:
            ...
            yield self._parse_host_analysis(full_text)
    ...
    def _normalize_args(obj: Any) -> Any:
        ...
        if hasattr(obj, "items"):
            return {
                str(k): HostAnalyzerAgent._normalize_args(v)
                for k, v in obj.items()
            }
        if hasattr(obj, "__iter__"):
            return [
                HostAnalyzerAgent._normalize_args(v)
                for v in obj
            ]
        return obj
    ...
    def _parse_host_analysis(
        self, text: str,
    ) -> dict[str, Any]:
        """
        Parse LLM text output into a structured host analysis.
        ...
        :return: a dict with type host_analysis and fields
        """
        cleaned = re.sub(
            r"^```(?:json)?\s*\n?", "", text.strip(),
        )
