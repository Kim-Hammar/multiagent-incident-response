"""
CodeVerifierAgent — uses Gemini with function calling to review
...
from response_planner_backend.agents.code_verifier_agent.tool_declarations import (
    ALL_DECLARATIONS,
...
class CodeVerifierAgent:
    ...
    def _normalize_args(obj: Any) -> Any:
        ...
        if hasattr(obj, "items"):
            return {
                str(k): CodeVerifierAgent._normalize_args(v)
                for k, v in obj.items()
            }
        if hasattr(obj, "__iter__"):
            return [
                CodeVerifierAgent._normalize_args(v)
                for v in obj
            ]
        return obj
