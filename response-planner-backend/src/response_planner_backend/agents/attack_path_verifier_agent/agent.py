...
from response_planner_backend.agents.attack_path_verifier_agent.tool_declarations import (
    TOOL_DECLARATIONS,
)
...
def _build_initial_message(
    ...
    """
    text = (
        "Please execute the described attack path "
        "step-by-step on the digital twin, starting "
...
class AttackPathVerifierAgent:
    ...
    def step_stream(
        ...
        prompt_kwargs: dict[str, Any] = dict(
            ...
            attack_path=attack_path or "N/A",
        )
        if dt_enabled:
            prompt_kwargs.update(
                attacker_info=format_attacker_info(cfg),
                dt_container_list=(
                    format_container_list_with_attacker(
                        cfg,
                    )
    ...
    def _build_contents(
        ...
        nudge = (
            ...
            "Analyze this result, then continue "
            "with the next step of the attack path."
        )
