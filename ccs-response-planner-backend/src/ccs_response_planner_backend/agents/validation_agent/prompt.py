...
### Response Plan
This plan was generated by an upstream pipeline: a Code Agent \
produced a Markov Decision Process (MDP) model of the incident \
recovery process, and an RL Agent trained a policy on that model \
to find an optimal sequence of response actions. The plan below \
is the action sequence recommended by the learned policy.
{{response_plan}}
...
used, a summary of training convergence, and the action sequence \
that forms the response plan above.
