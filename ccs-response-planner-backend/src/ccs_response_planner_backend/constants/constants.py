...
class EXAMPLES:
    ...
    ATTACK_PATH = (
        ...
        "code execution by uploading a shared library to a "
        "writable Samba share and then loading it via a "
        "specially crafted IPC pipe name. Use Metasploit "
        "(exploit/linux/samba/is_known_pipename) or a "
        "standalone Python exploit to upload a payload .so "
        "to a writable share on Server 6 and trigger "
        "execution, obtaining a root shell.\n"
        ...
        "users--) to extract data from the PostgreSQL "
